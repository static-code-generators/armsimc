...
{
    if (condition_check(get_bits(instruction, 31, 28))) {
        uint32_t data = mem_read_32(ld_str_addr_mode(instruction));
...
static void exec_STR(uint32_t instruction)
{
    if (condition_check(get_bits(instruction, 31, 28))) {
        ...
        mem_write_32(ld_str_addr_mode(instruction), data);
    }
}
...
static void exec_STRB(uint32_t instruction)
{
    if (condition_check(get_bits(instruction, 31, 28))) {
        ...
        mem_write_8(ld_str_addr_mode(instruction), data);
    }
}
